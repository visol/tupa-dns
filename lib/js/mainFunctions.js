...
//*******************************
// Multi Move Delete functions
//*******************************
function multiMvDelChkbUpdate(elem, move, del) {
//	multiMvDelResetDelCountMsg();
...
function updateMultiMvDelBtn() {
	...
	var mvBtn = d.getElementById('mmove');
	var delBtn = d.getElementById('mdel');
//	var delOnClick = d.getElementById('mdel').attributes['onclick'].nodeValue.replace(/%count%/, delCount);
//	d.getElementById('mdel').attributes['onclick'].nodeValue = delOnClick;
...
//	multiMvDelResetDelCountMsg();
	...
	var chkbState = conf['data']['id'] == undefined || idList.length != conf['data']['id'].length ? true : false;
	...
	updateMultiMvDelBtn();
	var elem = d.getElementsByName('mmvdel');
	for(i=0; i<elem.length; i++) {
		elem[i].checked = chkbState;
	}
}
function multiMvDelResetDelCountMsg() {
	var delOnClick = d.getElementById('mdel').attributes['onclick'].nodeValue.replace(delCount, '%count%');
	d.getElementById('mdel').attributes['onclick'].nodeValue = delOnClick;
}
function multiMvDelExecDel(msg, ctrue) {
	var msg = msg.replace(/%count%/, delCount);
	openConfirm(msg, ctrue, 'void(0);');
}
	...
	if (partsArr != '') {
		for(var i = 0;i<partsArr.length;i++) {
			...
			if (typeof(changeField) != undefined && typeof(changeField) != 'undefined') {
				...
				if (typeof(fieldArr[2]) != undefined && typeof(fieldArr[2]) != 'undefined') {
					//  && fieldArr[2] != ''
