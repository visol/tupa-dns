...
/**
 * checks for valid browser to use with TUPA. Return nothing or a JavaScript message
 *
  * @return	mixed
 * @package 	TUPA
...
function browserCheck() {
	...
	switch ($bro) {
		case 'moz':
			$allowedVer = array('1.7', '1.8');
			break;
		case 'op':
			if (substr_count($ver, '7.')) $message = $LANG->getLang('broCheckOp7');
			$allowedVer = array('8.');
			break;
		case 'ie':
			if ($sys == 'mac') $message = $LANG->getLang('broCheckMacIe');
			$allowedVer = array('6.', '7.');
			break;
		case 'saf':
			if (substr_count($ver, '2.')) $message = $LANG->getLang('broCheckSaf');
			break;
		case 'konq':
			$message = $LANG->getLang('broCheckKonq');
			break;
	}
	if ($message) return $message;
	if (isAllowedVer($ver, $allowedVer)) return '';
	return $LANG->getLang('broCheckNotTested');
}

function isAllowedVer($ver, $allowedVerArr) {
	if (is_array($allowedVerArr)) {
		foreach ($allowedVerArr as $value) {
			if (substr_count($ver, $value)) return true;
		}
	}
	return false;
}
 ...
 * @return	mixed
 * @package 	TUPA
 * @author 	Harald Hope <dont@know.it>
 */
 ...
 * @param 	string		browser version number
 * @return	string		OS
 * @package 	TUPA
 * @author 	Harald Hope <dont@know.it>
 */
